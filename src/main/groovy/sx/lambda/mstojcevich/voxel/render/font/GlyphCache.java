 ...
 *       font for rendering the GUI at its smallest size and for use in the F3
 *       debug screen; may need some explicit argument in
 *       StringCache.renderString() to select the size
 * @todo Need to have a config file that allows overring the font search order
 *       by locale to properly support Traditional Chinese hanzi, Simplified
...
public class GlyphCache {
     ...
     * the cache texture. This border keeps neighboring glyphs from
     * "bleeding through" when the scaled GUI resolution is not pixel aligned
     * and sometimes results in off-by-one sampling of the glyph cache textures.
     */
     ...
     */
    private BufferedImage glyphCacheImage = new BufferedImage(TEXTURE_WIDTH,
            TEXTURE_HEIGHT, BufferedImage.TYPE_INT_ARGB);

    /**
     * The Graphics2D associated with glyphCacheImage and used for bit blitting
     * between stringImage.
     */
    private Graphics2D glyphCacheGraphics = this.glyphCacheImage
            .createGraphics();
     ...
     */
    private FontRenderContext fontRenderContext = this.glyphCacheGraphics
     ...
     */
    private IntBuffer imageBuffer = ByteBuffer
     ...
     */
    private IntBuffer singleIntBuffer = ByteBuffer.allocateDirect(1 << 2).order(ByteOrder.nativeOrder()).asIntBuffer();

    /**
     * List of all available physical fonts on the system. Used by lookupFont()
     * to find alternate fonts.
     */
    private List<Font> allFonts = Arrays.asList(GraphicsEnvironment
     ...
     *
     * @param renderEngine
     *            needed to allocate OpenGL textures
     */
    public GlyphCache() {
		...
		/*
		 * The drawImage() to this buffer will copy all source pixels instead of
		 * alpha blending them into the current image
		 */
        this.glyphCacheGraphics.setComposite(AlphaComposite.Src);

        this.allocateGlyphCacheTexture();
        this.allocateStringImage(STRING_WIDTH, STRING_HEIGHT);

		/*
		 * Use Java's logical font as the default initial font if user does not
		 * override it in some configuration file
		 */
        java.awt.GraphicsEnvironment.getLocalGraphicsEnvironment()
                .preferLocaleFonts();
        this.usedFonts.add(new Font(Font.SANS_SERIF, Font.PLAIN, 1));
    }
     ...
     *            the new point size
     */
    void setDefaultFont(Font font, boolean antiAlias) {
     ...
     */
    GlyphVector layoutGlyphVector(Font font, char text[], int start, int limit,
                                  int layoutFlags) {
		/*
		 * Ensure this font is already in fontCache so it can be referenced by
		 * cacheGlyphs() later on
		 */
        if (!this.fontCache.containsKey(font)) {
            this.fontCache.put(font, this.fontCache.size());
        }
        return font.layoutGlyphVector(this.fontRenderContext, text, start,
                limit, layoutFlags);
    }

    public int getStringHeight(Font font, String str) {
        char[] text = str.toCharArray();
		/*
		 * Create new GlyphVector so glyphs can be moved around (kerning
		 * workaround; see below) without affecting caller
		 */
        GlyphVector vector = this.layoutGlyphVector(font, text, 0,
                str.length(), 0);

		/*
		 * Pixel aligned bounding box for the entire vector; only set if the
		 * vector has to be drawn to cache a glyph image
		 */
        Rectangle vectorBounds = null;

		/*
		 * This forms the upper 32 bits of the fontCache key to make every
		 * font/glyph code point unique
		 */
        long fontKey = (long) this.fontCache.get(font) << 32;

        int numGlyphs = vector.getNumGlyphs(); /* Length of the GlyphVector */
        Rectangle dirty = null; /*
								 * Total area within texture that needs to be
								 * updated with glTexSubImage2D()
								 */
        boolean vectorRendered = false; /*
										 * True if entire GlyphVector was
										 * rendered into stringImage
										 */

        for (int index = 0; index < numGlyphs; index++) {
			/*
			 * If this glyph code is already in glyphCache, then there is no
			 * reason to pre-render it again
			 */
			...
			/*
			 * The only way to get glyph shapes with font hinting is to draw the
			 * entire glyph vector into a temporary BufferedImage, and then bit
			 * blit the individual glyphs based on their bounding boxes returned
			 * by the glyph vector. Although it is possible to call
			 * font.createGlyphVector() with an array of glyphcodes (and
			 * therefore render only a few glyphs at a time), this produces
			 * corrupted Davengari glyphs under Windows 7. The vectorRendered
			 * flag will draw the string at most one time.
			 */
            if (!vectorRendered) {
                vectorRendered = true;

				/*
				 * Kerning can make it impossible to cleanly separate adjacent
				 * glyphs. To work around this, each glyph is manually advanced
				 * by 2 pixels to the right of its neighbor before rendering the
				 * entire string. The getGlyphPixelBounds() later on will return
				 * the new adjusted bounds for the glyph.
				 */
				...
				/*
				 * Compute the exact area that the rendered string will take up
				 * in the image buffer. Note that the string will actually be
				 * drawn at a positive (x,y) offset from (0,0) to leave enough
				 * room for the ascent above the baseline and to correct for a
				 * few glyphs that appear to have negative horizontal bearing
				 * (e.g. U+0423 Cyrillic uppercase letter U on Windows 7).
				 */
                vectorBounds = vector.getPixelBounds(this.fontRenderContext, 0,
                        0);

				/*
				 * Enlage the stringImage if it is too small to store the entire
				 * rendered string
				 */
                if (this.stringImage == null
                        || vectorBounds.width > this.stringImage.getWidth()
                        || vectorBounds.height > this.stringImage.getHeight()) {
                    int width = Math.max(vectorBounds.width,
                            this.stringImage.getWidth());
                    int height = Math.max(vectorBounds.height,
                            this.stringImage.getHeight());
                    this.allocateStringImage(width, height);
                }

				/* Erase the upper-left corner where the string will get drawn */
                this.stringGraphics.clearRect(0, 0, vectorBounds.width,
                        vectorBounds.height);

				/*
				 * Draw string with opaque white color and baseline adjustment
				 * so the upper-left corner of the image is at (0,0)
				 */
                this.stringGraphics.drawGlyphVector(vector, -vectorBounds.x,
                        -vectorBounds.y);
            }

			/*
			 * Get the glyph's pixel-aligned bounding box. The JavaDoc claims
			 * that the "The outline returned by this method is positioned
			 * around the origin of each individual glyph." However, the actual
			 * bounds are all relative to the start of the entire GlyphVector,
			 * which is actually more useful for extracting the glyph's image
			...
			/*
			 * If the current line in cache image is full, then advance to the
			 * next line
			 */
            if (this.cachePosX + rect.width + GLYPH_BORDER > TEXTURE_WIDTH) {
                this.cachePosX = GLYPH_BORDER;
                this.cachePosY += this.cacheLineHeight + GLYPH_BORDER;
                this.cacheLineHeight = 0;
            }

			/*
			 * If the entire image is full, update the current OpenGL texture
			 * with everything changed so far in the image (i.e. the dirty
			 * rectangle), allocate a new cache texture, and then continue
			 * storing glyph images to the upper-left corner of the new texture.
			 */
            if (this.cachePosY + rect.height + GLYPH_BORDER > TEXTURE_HEIGHT) {
                this.updateTexture(dirty);
                dirty = null;

				/*
				 * Note that allocateAndSetupTexture() will leave the GL texture
				 * already bound
				 */
                this.allocateGlyphCacheTexture();
                this.cachePosY = this.cachePosX = GLYPH_BORDER;
                this.cacheLineHeight = 0;
            }

			/*
			 * The tallest glyph on this line determines the total vertical
			 * advance in the texture
			 */
     ...
     *
     * @param text
     *            the string to check against the font
     * @param start
     *            the offset into text at which to start checking characters for
     *            being supported by a font
     * @param limit
     *            the (offset + length) at which to stop checking characters
     * @param a
     *            combination of the Font.PLAIN, Font.BOLD, and Font.ITALIC to
    ...
    Font lookupFont(char text[], int start, int limit, int style) {
		/*
		 * Try using an already known base font; the first font in usedFonts
		 * list is the one set with setDefaultFont()
		 */
        Iterator<Font> iterator = this.usedFonts.iterator();
        while (iterator.hasNext()) {
			/*
			 * Only use the font if it can layout at least the first character
			 * of the requested string range
			 */
            Font font = iterator.next();
            if (font.canDisplayUpTo(text, start, limit) != start) {
				/*
				 * Return a font instance of the proper point size and style;
				 * usedFonts has only 1pt sized plain style fonts
				 */
		...
		/*
		 * If still not found, try searching through all fonts installed on the
		 * system for the first that can layout this string
		 */
        ...
        while (iterator.hasNext()) {
            ...
            if (font.canDisplayUpTo(text, start, limit) != start) {
				/*
				 * If found, add this font to the usedFonts list so it can be
				 * looked up faster next time
				 */
                this.usedFonts.add(font);

				/*
				 * Return a font instance of the proper point size and style;
				 * allFonts has only 1pt sized plain style fonts
				 */
                return font.deriveFont(style, this.fontSize);
            }
        }
		...
		/*
		 * If no supported fonts found, use the default one (first in usedFonts)
		 * so it can draw its unknown character glyphs
		 */
        Font font = this.usedFonts.get(0);

		/*
		 * Return a font instance of the proper point size and style; usedFonts
		 * only 1pt sized plain style fonts
		 */
        return font.deriveFont(style, this.fontSize);
    }
     ...
     *
     * @param font
     *            the font to which this glyphCode belongs and which was used to
     *            pre-render the glyph image in cacheGlyphs()
     * @param glyphCode
     *            the font specific flyph code to lookup in the cache
     ...
     *
     * @param font
     *            the font used to create a GlyphVector for the string and to
     *            actually draw the individual glyphs
     * @param text
     *            the string from which to cache glyph images
     * @param start
     *            the offset into text at which to start caching glyphs
     * @param limit
     *            the (offset + length) at which to stop caching glyphs
     * @param layoutFlags
     *            either Font.LAYOUT_RIGHT_TO_LEFT or Font.LAYOUT_LEFT_TO_RIGHT;
     *            needed for weak bidirectional characters like parenthesis
     *            which are mapped to two different glyph codes depending on the
     ...
     */
    void cacheGlyphs(Font font, char text[], int start, int limit,
                     int layoutFlags) {
		/*
		 * Create new GlyphVector so glyphs can be moved around (kerning
		 * workaround; see below) without affecting caller
		 */
        GlyphVector vector = this.layoutGlyphVector(font, text, start, limit,
                layoutFlags);
        ...
        for (int index = 0; index < numGlyphs; index++) {
			...
			/*
			 * Blit the individual glyph from it's position in the temporary
			 * string buffer to its (cachePosX, cachePosY) position in the
			 * texture. NOTE: We don't have to erase the area in the texture
			 * image first because the composite method in the Graphics object
			 * is always set to AlphaComposite.Src.
			 */
            this.glyphCacheGraphics.drawImage(this.stringImage, this.cachePosX,
                    this.cachePosY, this.cachePosX + rect.width, this.cachePosY
                            + rect.height, rect.x, rect.y, rect.x + rect.width,
                    rect.y + rect.height, null);

			/*
			 * Store this glyph's position in texture and its origin offset.
			 * Note that "rect" will not be modified after this point, and
			 * getGlyphPixelBounds() always returns a new Rectangle.
			 */
            rect.setLocation(this.cachePosX, this.cachePosY);

			/*
			 * Create new cache entry to record both the texture used by the
			 * glyph and its position within that texture. Texture coordinates
			 * are normalized to 0.0-1.0 by dividing with TEXTURE_WIDTH and
			 * TEXTURE_HEIGHT.
			 */
			...
			/*
			 * The lower 32 bits of the glyphCache key are the glyph codepoint.
			 * The upper 64 bits are the font number stored in the fontCache.
			 * This creates a unique numerical id for every font/glyph
			 * combination.
			 */
            this.glyphCache.put(fontKey | glyphCode, entry);

			/*
			 * Track the overall modified region in the texture by performing a
			 * union of this glyph's texture position with the update region
			 * created so far. Reusing "rect" here makes it easier to extend the
			 * dirty rectangle region than using the add(x, y) method to extend
			 * by a single point. Also note that creating the first dirty
			 * rectangle here avoids having to deal with the special rules for
			 * empty/non-existent rectangles.
			 */
            if (dirty == null) {
                dirty = new Rectangle(this.cachePosX, this.cachePosY,
			...
			/*
			 * Advance cachePosX so the next glyph can be stored immediately to
			 * the right of this one
			 */
            this.cachePosX += rect.width + GLYPH_BORDER;
        }
     ...
     *
     * @param dirty
     *            The rectangular region in glyphCacheImage that has changed and
     *            needs to be copied into the texture
     *
    ...
    private void updateTexture(Rectangle dirty) {
        ...
        if (dirty != null) {
			/*
			 * Load imageBuffer with pixel data ready for transfer to OpenGL
			 * texture
			 */
            ...
            GL11.glTexSubImage2D(GL11.GL_TEXTURE_2D, 0, dirty.x, dirty.y,
                    ...
                    GL11.GL_UNSIGNED_BYTE, this.imageBuffer);
        }
    }
    ...
    private void allocateStringImage(int width, int height) {
        this.stringImage = new BufferedImage(width, height,
		...
		/*
		 * Full white (1.0, 1.0, 1.0, 1.0) can be modulated by vertex color to
		 * produce a full gamut of text colors, although with a GL_ALPHA8
		 * texture, only the alpha component of the color will actually get
    ...
    private void setRenderingHints() {
        this.stringGraphics.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
                this.antiAliasEnabled ? RenderingHints.VALUE_ANTIALIAS_ON
                        : RenderingHints.VALUE_ANTIALIAS_OFF);
        this.stringGraphics.setRenderingHint(
                RenderingHints.KEY_TEXT_ANTIALIASING,
                this.antiAliasEnabled ? RenderingHints.VALUE_TEXT_ANTIALIAS_ON
    ...
    private void allocateGlyphCacheTexture() {
        ...
        this.glyphCacheGraphics.clearRect(0, 0, TEXTURE_WIDTH, TEXTURE_HEIGHT);
        ...
        this.updateImageBuffer(0, 0, TEXTURE_WIDTH, TEXTURE_HEIGHT);

		/*
		 * Initialize texture with the now cleared BufferedImage. Using a
		 * texture with GL_ALPHA8 internal format may result in faster rendering
		 * since the GPU has to only fetch 1 byte per texel instead of 4 with a
		...
		/*
		 * Explicitely disable mipmap support becuase updateTexture() will only
		 * update the base level 0
		 */
     ...
     *
     * @param x
     *            the horizontal coordinate of the region's upper-left corner
     * @param y
     *            the vertical coordinate of the region's upper-left corner
     * @param width
     *            the width of the pixel region that will be copied into the
     *            buffer
     * @param height
     *            the height of the pixel region that will be copied into the
     *            buffer
     */
    private void updateImageBuffer(int x, int y, int width, int height) {
		/*
		 * Copy raw pixel data from BufferedImage to imageData array with one
		 * integer per pixel in 0xAARRGGBB form
		 */
        this.glyphCacheImage.getRGB(x, y, width, height, this.imageData, 0,
                width);
		...
		/*
		 * Copy int array to direct buffer; big-endian order ensures a 0xRR,
		 * 0xGG, 0xBB, 0xAA byte layout
		 */
